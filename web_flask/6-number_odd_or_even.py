...
"""Module to make a simple Flask web application with:
    ...
    - route that renders a Jinja2 template HTML with conditionals
"""
...
def is_number_template(n):
    """Helper function to serve a HTML template page only if n is an integer
    parameter.


    Take in account that this function will be triggered
    with or without trailing slashes, it means that it will be available at:
    '0.0.0.0:5000/number_template/<int:n>' or
    '0.0.0.0:5000/number_template/<int:n>/'

    Arguments:
        n (int):    An integer that will be rendered on the template HTML
                    response.
...
def is_number_template_odd_even(n):
    ...
    The template HTML indicates if the number its odd or even.

    Take in account that this function will be triggered
    with or without trailing slashes, it means that it will be available at:
    '0.0.0.0:5000/number_odd_or_even/<int:n>' or
    '0.0.0.0:5000/number_odd_or_even/<int:n>/'
